...
from app.models.surgery import SurgeryDisease, SurgeryFormula, SurgerySyndrome, SurgeryTreatmentRule
...
router = APIRouter(prefix="/api/v1/surgery/treatment", tags=["疮疡-辨证论治"])
...
async def _experience_formula_refs(
    disease: SurgeryDisease, syndrome, db: AsyncSession
) -> list[FormulaOut]:
    ...
    stmt = (
        select(SurgeryFormula)
        .where(
            SurgeryFormula.source == "《中医外科经验论集》",
            SurgeryFormula.usage_type.in_(["内服", "内服外用"]),
        )
        .order_by(SurgeryFormula.id)
    )
    ...
    def _score(f: SurgeryFormula) -> int:
...
async def _external_formula_refs(
    stage: str, is_yang: bool, db: AsyncSession
) -> list[FormulaOut]:
    ...
    stmt = (
        select(SurgeryFormula)
        .where(
            SurgeryFormula.source == "《外科传薪集》",
            SurgeryFormula.usage_type.in_(["外用", "内服外用"]),
            SurgeryFormula.method == method,
        )
        .order_by(SurgeryFormula.id)
    )
    result = await db.execute(stmt)
    ...
    def _is_warm(f: SurgeryFormula) -> bool:
...
async def recommend(body: TreatmentRecommendIn, db: AsyncSession = Depends(get_db)):
    disease = await db.get(SurgeryDisease, body.disease_id)
    if not disease:
        raise HTTPException(status_code=404, detail="病种不存在")

    syndrome = None
    if body.syndrome_id:
        syndrome = await db.get(SurgerySyndrome, body.syndrome_id)

    stmt = (
        select(SurgeryTreatmentRule)
        .options(
            selectinload(SurgeryTreatmentRule.syndrome),
            selectinload(SurgeryTreatmentRule.formula),
        )
        .where(
            SurgeryTreatmentRule.disease_id == body.disease_id,
            SurgeryTreatmentRule.stage == body.stage,
        )
    )
    result = await db.execute(stmt)
...
def _score(syndrome: SurgerySyndrome, symptoms: list[str]) -> int:
    keywords = _SYNDROME_KEYWORDS.get(syndrome.name, [])
    return sum(_kw_weight(k) for k in keywords if k in symptoms)


def _matched(syndrome: SurgerySyndrome, symptoms: list[str]) -> list[str]:
    keywords = _SYNDROME_KEYWORDS.get(syndrome.name, [])
...
async def differentiate(body: DifferentiateIn, db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(SurgerySyndrome).order_by(SurgerySyndrome.id))
    ...
    if body.disease_id and differentiation == "分型":
        rst = select(SurgeryTreatmentRule.syndrome_id).where(
            SurgeryTreatmentRule.disease_id == body.disease_id,
            SurgeryTreatmentRule.stage == "分型",
            SurgeryTreatmentRule.syndrome_id.isnot(None),
        )
        ...
        if body.disease_id:
            ...
            if body.stage:
                rst = rst.where(SurgeryTreatmentRule.stage == body.stage)
...
def _formula_match(formula: SurgeryFormula, keywords: list[str]) -> tuple[int, list[str]]:
...
async def match_formula(body: MatchFormulaIn, db: AsyncSession = Depends(get_db)):
    ...
    stmt = select(SurgeryFormula)
    if body.domain:
        stmt = stmt.where(SurgeryFormula.domain == body.domain)
    elif domains:
        stmt = stmt.where(SurgeryFormula.domain.in_(domains))
    result = await db.execute(stmt)
    formulas = list(result.scalars().all())
